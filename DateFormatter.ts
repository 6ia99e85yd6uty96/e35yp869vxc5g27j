class DateFormatter {
    ...
    public constructor(srcDate: Date) {
    ...
    public digitalDate(dateInputFormat: string = ""): string {
        let result;
        let dstDate = this._dateParser.parse(dateInputFormat, this._srcDate);
    ...
    public date(linguisticInfo: string = "", dateInputFormat: string = "") {
        ...
        let linguist = new Linguist(linguisticInfo, 'р');
        this.convertGrammaticalCase(linguist, false);
        let dstDate = this._dateParser.parse(dateInputFormat, this._srcDate);
        let dictionary = this._months[this._language][dstDate.getMonth()];
        if (this._language == 'ru') {
    ...
    public weekDay(linguisticInfo: string = "", dateInputFormat: string = "") {
        ...
        let dstDate = this._dateParser.parse(dateInputFormat, this._srcDate);
        let dictionary = this._weekDays[this._language][dstDate.getDay()];
        return linguist.declinate(dictionary);
    }

    private wereArgumentsSwapped(linguisticInfo: string, dateInputFormat: string): boolean {
        return !Linguist.looksLikeLinguisticInfo(linguisticInfo)
            && (!dateInputFormat || Linguist.looksLikeLinguisticInfo(dateInputFormat));
    }

    private convertGrammaticalCase(linguist: Linguist, allowPlural: boolean) {
    ...
    private addLeadZero(str: string|number): string {
        str = str.toString();
        if (str.length == 1) {
            str = '0' + str;
        }
